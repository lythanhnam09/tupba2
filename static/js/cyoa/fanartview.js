...
async function showImageView(cyoaId, page, pagecount, total, perpage, index) {
    if (imgLoader == null) {
        imgLoader = new PagePreLoader(perpage, (page, perpage) => socketEmitGet('cyoa_fanart_data', {page:page, perpage:perpage, cyoaId:cyoaId}, true));
    }
    perPage = perpage;
    currentIndex = index;
    totalCount = total;
...
function refreshCyoaFanart(cyoaId) {
    socket.emit('cyoa_fanart_refresh', {id:cyoaId});
    $('#btn-refresh').html('<i class="fas fa-ellipsis-h"></i> Please wait');
