...
class BooruSearchPreloader(task_util.Preloader):
    ...
    async def run_request(self, enum):
        ...
        link = f'https://derpibooru.org/api/v1/json/search/images?q={qp.export_query()}&filter_id={booru_config.data["booru_filter_id"]}&page={enum}&perpage={self.meta["perpage"]}&sf={self.meta["sf"]}&sd={self.meta["sd"]}'
        print(link)
        ...
        BooruImage.insert(page_data.data, update_conflict=True, set_col=['upvote', 'downvote', 'score', 'fave', 'comment_count', 'wilson_score', 'link_source', 'updated_at', 'description'], conn=conn)
        ...
        for img in page_data.data:
            img.get_ref('tags', save_result=True, conn=conn)
            img.get_ref('sizes', save_result=True, conn=conn)
