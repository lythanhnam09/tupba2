...
def refresh_thread_list(cy:cyoa.Cyoa):
    ...
    if (len(ls_emty) > 0):
        ...
        qw = task_util.QueueWorker(name=f'Refresh thread data: {cy["name"]}', dequeue_on_done = True, meta={'id':cy['short_name'], 'category':'CYOA'})
        for th in ls_emty:
        ...
        qw = task_util.QueueWorker(name=f'Refresh last thread data: {cy["name"]}', dequeue_on_done = True, meta={'id':cy['short_name'], 'category':'CYOA'})
