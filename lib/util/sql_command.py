            ...
            if (type(expr) == list):
                ...
                if (len(expr) ==  1):
                    ...
                    if (expr[1] in ['=', '<>', '>', '<', 'in', 'like']):
                        ...
                        if (type(expr[1]) in [int, float, str]):
                            ls_expr.append(f'{expr[0]} = {expr[1]!r}')
...
def select_join(table:list, join_on:str, columns:list = None, where:list = None, group_by:list = None, having:list = None, order_by:list = None, limit:int = None, offset:int = None, line_break = False) -> str:
    ...
    if (len(table) == 2):
        result += ('\n' if line_break else ' ') + f'from {table[0]} join {table[1]} on {join_on}'
    else:
...
def select_join_3(table:list, join_name:str, join_on:list, columns:list = None, where:list = None, group_by:list = None, having:list = None, order_by:list = None, limit:int = None, offset:int = None, line_break = False) -> str:
    result = ''
    ...
    if (len(table) == 3):
        result += ('\n' if line_break else ' ') + f'from ({table[0]} join {table[1]} on {join_on[0]}) {join_name} join {table[2]} on {join_on[1]}'
    else:
        raise Exception('The number of join table in list must be 3, otherwise better use select() or select_join()')
